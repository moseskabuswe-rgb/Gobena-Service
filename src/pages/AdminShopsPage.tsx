...
import {
  Plus, Store, Search, ChevronRight, CheckCircle, Clock, X, AlertCircle, MapPin, Mail, Phone,
...
export default function AdminShopsPage() {
  ...
  const loadShops = async () => {
    const { data } = await supabase
      .from('shops')
      .select('id, name, address, city, state, contact_name, contact_email, contact_phone, status, approved_at, notes, created_at')
        ...
        <div className="flex gap-2">
          {(['all','approved','pending','suspended'] as const).map(s => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3.5 py-1.5 rounded-full text-xs font-medium transition border ${filter === s ? 'bg-stone-900 text-white border-stone-900' : 'bg-white text-stone-500 border-stone-200'}`}>
          ...
          <div className="space-y-2">
            {filtered.map(shop => (
              <button key={shop.id} onClick={() => setSelected(shop)}
                className="w-full bg-white rounded-2xl border border-stone-100 px-4 py-4 text-left hover:border-stone-200 hover:shadow-sm transition active:scale-[.99]">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              <div className="flex items-center gap-2">
                ...
                {selected.approved_at && (
                  <span className="text-xs text-stone-400">Approved {new Date(selected.approved_at).toLocaleDateString()}</span>
                )}
              </div>
              ...
              {selected.notes && (
                <p className="text-sm text-stone-500 bg-stone-50 rounded-xl px-3 py-2.5">{selected.notes}</p>
              )}

              {/* Actions */}
              <div className="space-y-2 pt-2">
                <Link
                  to={`/admin/shops/${selected.id}/equipment`}
                  className="block w-full py-2.5 text-center text-sm font-semibold bg-stone-900 text-white rounded-xl hover:bg-stone-800 transition"
                >
                  Manage equipment
                </Link>
                ...
                {selected.status === 'approved' && (
                  <button
                    ...
                    className="w-full py-2.5 text-sm font-semibold bg-white text-red-600 border border-red-200 rounded-xl hover:bg-red-50 transition"
                  >
                    Suspend shop
                  </button>
                )}
          ...
          <div className="bg-white w-full md:max-w-lg md:rounded-2xl rounded-t-2xl max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleAddShop} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => set('notes')(e.target.value)} rows={2} placeholder="Any internal notes…"
