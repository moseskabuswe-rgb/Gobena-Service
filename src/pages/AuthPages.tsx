  ...
  return (
    <AuthShell title="Welcome back" sub="Sign in to Gobena Service">
      ...
      <p className="mt-5 text-center text-sm text-stone-500">
        New shop?{' '}
        <Link to="/register" className="text-amber-700 font-medium hover:underline">Request access</Link>
      </p>
    </AuthShell>
  );
}
...
export function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. Insert shop — uses "Anyone insert shop" policy (no auth check needed)
    const { data: shopData, error: shopErr } = await supabase
      .from('shops')
      .insert({
        name: form.shopName, address: form.address, city: form.city,
        state: form.state, contact_name: form.contactName,
    ...
    if (shopErr || !shopData) {
      setError('Shop registration failed: ' + (shopErr?.message || 'unknown error'));
      setLoading(false);
      return;
    }

    // 3. Insert profile — uses "Own profile insert" policy (auth.uid() = id)
    // signUp with implicit flow sets the session immediately so auth.uid() is available
    ...
    // Sign out — they need admin approval before logging in
      ...
      <AuthShell title="Request submitted" sub="We'll be in touch soon">
        <div className="text-center space-y-4">
          ...
          <div className="text-sm text-stone-600 leading-relaxed">
            <p className="font-semibold text-stone-800 mb-1">Request pending approval</p>
            <p>Gobena will review your shop and contact you once approved. This usually takes 1–2 business days.</p>
          </div>
          <Link to="/login"
